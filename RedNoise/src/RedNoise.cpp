...
std::vector<std::pair<ModelTriangle, Material>> loadObj(std::string path, float scale) {
	...
	while(!file.eof()) {
		...
		if(tokens[0].compare("mtllib") == 0) mtlPath = tokens[1];
			...
			if(material.texturePath.empty() && colourPack(triangle.colour,0xFF) == 0xFFFFFFFF) {
				std::cout << "light" << std::endl;
				lightSourceModels.push_back(triangle);
			}
		}
	}
...
int main(int argc, char *argv[]) {
	...
	lightSource = glm::vec3(0.0, 0.45, 0.0); //from (almost) center of white square
